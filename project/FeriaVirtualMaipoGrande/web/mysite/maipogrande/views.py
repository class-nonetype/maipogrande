...
def profile(request, username = None):
	...
	if username != current_user:
		...
		posts = user.product.all()

	else:

		posts = current_user.product.all()
		user = current_user
	
	if user.type == 'TRANSPORTISTA':
		context = {
			'user' : user,
			'posts' : Transport.objects.filter(user = user.id).all()
		}
...
###################################################################
@login_required
def generate_sales_transaction(request, id_bank_account = None, amount = None,
	...
	if bank_account.bank_amount <= 0:
		...
		if bank_account.bank_amount > amount:
			bank_account.bank_amount = bank_account.bank_amount - amount
		
		elif bank_account.bank_amount < amount:
			bank_account.bank_amount = amount - bank_account.bank_amount
		...
		bank_account = BankAccount.objects.filter(owner_full_name = producer_user_full_name).first()
...
@login_required
def client_requests_status(request, id_client : None):
...
def recharge_amount(request, id_bank : None, recharge_amount : None):
	...
	bank_account = BankAccount.objects.get(id_bank_account = id_bank)
